...
def no_grad_loop(data_loader, model, png_cnt, epoch=2, device="cuda", batch_size = 64):
    ...
    for i, (Pc, df, sdf) in enumerate(data_loader):
        ...
        if i == len(data_loader) -1:
            ...
            # #idx = np.random.permutation(len(pt_cloud))
            # #pt_cloud = pt_cloud[idx][:len(pt_cloud)//2]
            ...
            wandb.log({"Images": wandb.Image("train.png")}, commit=False)  #,"Point cloud target": target_wandb, "Point cloud prediction": pred_wandb }, commit=False)
            #wandb.log({"Point cloud target": target_wandb, "Point cloud prediction": pred_wandb }, commit=False)
            plt.close(fig)
            plt.close('all')
...
def train(model: NeuralNet, num_epochs, batch_size, train_loader, test_loader, validation_loader, learning_rate=1e-3, device="cuda"):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer, patience=8)
