...
def run():
    with wandb.init(project="3d_conv3d_fully_connected", entity="master-thesis-ntnu", config=dict) as run:
        config = wandb.config
        ...
        model = NeuralNet(config.layer_sizes+[dim*dim*dim*3], cnn_layer_sizes=[32,32,32], dropout=config.dropout).to(device)
        ...
        torch.save(config, "model_end_sweep.pth") 



if __name__ == "__main__":
    wandb.agent(id, project="3d_conv3d_fully_connected", entity="master-thesis-ntnu", function=run, count=100)